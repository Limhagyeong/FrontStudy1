...
public String food_date(HttpServletRequest request, HttpServletResponse response)
{
	// 예약 가능한 날
	...
	int[] rday=new int[32]; // => 1번부터 시작함 31+1
	st=new StringTokenizer(rdays,",");
	while(st.hasMoreTokens())
	{
		int a=Integer.parseInt(st.nextToken());
		// 2,7,8,9,11,12,13,15,16,18,20,23,27,28,29,30,31 => 잘라줌
		if(a>=day) // 금일자보다 큰 일자를 가지고옴
		{
			rday[a]=1; 
		}
	}
...
public String reserve_ok(HttpServletRequest request, HttpServletResponse response)
{
	...
	String fno=request.getParameter("fno");
	String rdate=request.getParameter("rday");
